...
function IndoorPage() {
  const officeData = servicebarLinks.find((link) => link.name === "Indoor");
  ...
  const handleSendEmail = () => {
    const mailtoLink = `mailto:tylervassalloconstruction@gmail.com?subject=Indoor Renovation&body=Name: ${formData.name}%0D%0A%0D%0APhone Number: ${formData.phoneNumber}%0D%0A%0D%0AAddress: ${formData.address}%0D%0A%0D%0ADescription: ${formData.description}`;
  ...
  return (
    <div className="bg-gradient-to-r from-gray-100 to-[#283132] min-h-screen flex flex-col items-center justify-center py-12">
      <h1 className="text-4xl font-bold mb-8 text-black">Indoor Services</h1>
